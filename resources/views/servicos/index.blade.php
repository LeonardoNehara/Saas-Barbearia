...
@section('content')
    <section class="rounded-3xl border border-slate-200 bg-white p-4 sm:p-7" aria-labelledby="servicos-title">
        <div class="mb-7 flex flex-wrap items-center justify-between gap-5">
            ...
            <a href="{{ route('servicos.create') }}" class="admin-button"><x-icon name="plus" class="size-4" />Novo Serviço</a>
        </div>
                    ...
                    @forelse ($servicos as $servico)
                        <tr class="hover:bg-slate-50/70">
                            ...
                            <td class="px-3 py-5"><div class="flex justify-end gap-1">
                                <a href="{{ route('servicos.edit', $servico) }}" aria-label="Editar {{ $servico->nome }}" title="Editar serviço" class="rounded-lg p-3 text-muted hover:bg-slate-100 hover:text-brand-dark"><x-icon name="edit" class="size-4" /></a>
